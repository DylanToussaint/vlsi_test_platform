...
- **DAC Configuration**: Configures the bias voltages for the PLL via I2C protocol
...
| `dac_commander` | `dac_commander.sv` | Manages I2C communication with the DAC for PLL bias voltage configuration |
...
> **Note**: The Quartus IP generator may not generate the PLL IP correctly. Verify that the output frequency matches the expected value before deployment. If the write/read report shows errors, the clock might be the cause. Also try to reset and reprogram the FPGA.

## I2C and DAC Commander

The DAC on the test board configures the PLL bias voltages via the I2C protocol. The `dac_commander` module orchestrates this configuration by sending commands through the `i2c_master` module.
...
For details on command sequences, refer to `dac_commander.sv`.
...
- **Preserve DAC Configuration**: The `dac_commander` module is required for correct PLL operation
